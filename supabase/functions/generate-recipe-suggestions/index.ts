...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { user_input, user_id: payload_user_id, is_pro, nutrition_goals } = await req.json() as RecipeSuggestionRequest;
    ...
    if (!user_input) {
      return new Response(
        JSON.stringify({ error: 'Missing required field: user_input' }),
        { 
          status: 400, 
